...
contract StopOnRevertHandler is Test {
    ...
    constructor(DSCEngine _dscEngine, DecentralizedStableCoin _dsc) {
        ...
        address[] memory collateralTokens = dscEngine.getTokenCollateralAddresses();
        weth = ERC20Mock(collateralTokens[0]);
        wbtc = ERC20Mock(collateralTokens[1]);

        ethUsdPriceFeed = MockV3Aggregator(dscEngine.getPriceFeedAddresses(address(weth)));
        btcUsdPriceFeed = MockV3Aggregator(dscEngine.getPriceFeedAddresses(address(wbtc)));
    }
    ...
    function redeemCollateral(uint256 collateralSeed, uint256 amountCollateral) public {
        ERC20Mock collateral = _getCollateralFromSeed(collateralSeed);
        uint256 maxCollateral = dscEngine.getCollateralDeposited(msg.sender, address(collateral));
    ...
    function liquidate(uint256 collateralSeed, address userToBeLiquidated, uint256 debtToCover) public {
        uint256 minHealthFactor = dscEngine.getMinimumHealthFactor();
    ...
    function updateCollateralPrice(uint96 newPrice, uint256 collateralSeed) public {
        ...
        ERC20Mock collateral = _getCollateralFromSeed(collateralSeed);
        MockV3Aggregator priceFeed = MockV3Aggregator(dscEngine.getPriceFeedAddresses(address(collateral)));
